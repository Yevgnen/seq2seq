...
print('loaded training source senstences: {0}, max length: {1}, vocabulary size: {2}'.format(
    len(train_x.get_value(borrow=True)), len(mask_train_x.get_value(borrow=True)), len(list(src_word2index))))
print('loaded training target senstences: {0}, max length: {1}, vocabulary size: {2}'.format(
...
def epoch_end_callback():
    ...
    for (source, predict, target) in zip(sample_x, predict_y, sample_y):
        ...
        print(' Source: {0}'.format(' '.join(source)))
        print(' Target: {0}'.format(' '.join(target)))
